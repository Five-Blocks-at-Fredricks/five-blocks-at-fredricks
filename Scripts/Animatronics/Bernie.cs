...
public partial class Bernie : Node3D {
    ...
    public override void _Ready() {
        ...
        BernieAiLevelLabel.Text = "Ai: " + Globals.BernieAiLevel.ToString();
    }
    ...
    public override void _Process(double delta) {
        ...
        if (MoveValue <= Globals.BernieAiLevel && MoveTimer >= 5f) {
            // Bernie's Starting Pos:
            // X: -2.607
            // Y: 3.089
            // Z: -44.654

            MoveBernie(ref PositionNumber, ref Pos, ref Rot, ref HeadRotation, true);
        }
    ...
    private void MoveBernie(ref int PositionNumber, ref Vector3 Pos, ref Vector3 Rot, ref Vector3 HeadRotation, bool MoveBackwardsChance) {
        if (MoveBackwardsChance && Random.Shared.NextDouble() < 0.3 && PositionNumber > 1 && PositionNumber != 5) {
            PositionNumber -= 2;
        }
